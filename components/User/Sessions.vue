<template>
  <DataTable :value="sessions" sort-field="expiresAt" :sort-order="1">
    <template #header>
      <h2 class="text-xl font-bold">{{ $t('activeSessions') }}</h2>
    </template>
    <template #empty>
      <div class="text-center text-red-400">{{ $t('noActiveSessions') }}</div>
    </template>
    <Column :header="$t('device')" class="max-w-20">
      <template #body="{ data: { platform, browser, ip, userAgent } }">
        <div class="flex flex-col">
          ...
          <span v-if="ip" class="flex gap-1"
            ><span class="font-semibold whitespace-nowrap">{{ `${$t('ip')}:` }}</span> <span class="truncate">{{ ip }}</span></span
          >
          <span v-if="userAgent" class="flex gap-1"
            ><span class="font-semibold whitespace-nowrap">{{ `${$t('userAgent')}:` }}</span>
            <span class="truncate">{{ userAgent }}</span></span
          >
        </div>
      </template>
    </Column>
    <Column field="expiresAt" :header="$t('expiresAt')" sortable class="truncate max-w-10">
